// app.js — coordinator: builds the project, wires model + preview + timeline +
// inspector + bridge together, and owns the toolbar and the keyboard.
//
// Readouts and the play button live in transport.js; everything that crosses
// the bridge lives in host-sync.js.
import {
  newProject, addAsset, addClip, getTrack, getAsset, findClip,
  pruneEmptyTracks, addTrackForType, overlayPlacement,
} from './model.js';
import { rippleMain, splitClip, deleteClip, playheadAfterRipple } from './timeline.js';
import { Preview } from './preview.js';
import { TimelineUI } from './timeline-ui.js';
import { Inspector } from './inspector.js';
import { History } from './history.js';
import { send, onMessage } from './bridge.js';
import { mediaUrl, refreshAssets, importAssets, importBroll, onBrollClick, pasteBroll, loadTrims } from './assets.js';
import { previewUrl } from './preview-source.js';
import { TrimPanel } from './trim-panel.js';
import { Transport } from './transport.js';
import { installHostSync } from './host-sync.js';

// `let`, not `const`: on projectLoaded we swap in a freshly-loaded project.
// Every function below that reads the bare `project` binding reads it live at
// call time, so reassigning here - kept in lockstep with app.project - is
// enough to make them all operate on the loaded project.
let project = newProject('9:16');

// Video plays from a small preview proxy when the host has built one, and from
// the master until then. Only the PREVIEW ever uses it; every render reads the
// master. See preview-source.js.
const assetUrl = (assetId) => {
  const asset = getAsset(project, assetId);
  if (!asset) return '';
  return asset.type === 'video' ? previewUrl(asset.path) : mediaUrl(asset.path);
};
...
const app = {
  ...
  onPlayheadChange() { if (app.transport) app.transport.update(); },

  pausePlayback() { if (app.transport) app.transport.pause(); },

  commit() {
    app.timeline.render();
    app.refreshPreview();
    if (app.transport) app.transport.update();
  },

  togglePlay() { if (app.transport) app.transport.toggle(); },
  ...
  pushHistory() { app.history.push(project); },
  ...
  // Used by host-sync when a saved project comes back from the app.
  loadProject(loaded) {
    app.pausePlayback();
    project = loaded;
...
app.transport = new Transport(app, {
  ...
  time: document.getElementById('transport-time'),
...
const host = installHostSync(app, { statusText, statusPill, btnExport });
...
// project: video/audio via a temp element and `loadedmetadata`, images via a
// temp Image(). Images get a default 5s duration since a still has no length.
async function probeMedia(assetInfo) {
  const url = assetInfo.type === 'video' ? previewUrl(assetInfo.path) : mediaUrl(assetInfo.path);
  ...
  return { duration: el.duration, naturalW: el.videoWidth || 0, naturalH: el.videoHeight || 0 };
}
...
async function addAssetAndClip(assetInfo, trackId, dropTime, trim) {
  ...
  const assetId = addAsset(project, {
    path: assetInfo.path, type: assetInfo.type, naturalW, naturalH, duration,
  });
  ...
  const clipId = addClip(project, trackId, {
    assetId, start: Math.max(0, dropTime), in: inPoint, duration: clipLen,
  });

  const track = getTrack(project, trackId);
  if (track && track.kind === 'main') {
    rippleMain(track);
  } else if (track && track.kind === 'overlay') {
    // Land it as a picture ON the video, not over the whole frame. Without
    // this an overlay draws at its own pixel size, which for any real photo is
    // far bigger than the canvas.
    const placed = overlayPlacement(project.canvas, naturalW, naturalH);
    const found0 = findClip(project, clipId);
    if (found0) Object.assign(found0.clip, placed);
  }

  // Preview only knows about assets that existed at the last setProject() - run
  // it again so the new one has an element before commit() asks for a frame.
  preview.setProject(project);
...
function doSplit(clipId) {
  ...
  app.commit();       // the playhead is already exactly on the cut
}

// Deleting from the magnetic main track pulls everything after it back. The
// playhead comes too, so you keep looking at the same frame instead of watching
// the picture slide out from under a stationary line.
function doDelete(clipId) {
  ...
  if (magnetic) {
    app.playhead = playheadAfterRipple(app.playhead, start, -duration);
...
document.getElementById('btn-zoom-in').addEventListener('click', () => app.timeline.zoom(1));
document.getElementById('btn-zoom-out').addEventListener('click', () => app.timeline.zoom(-1));
...
document.getElementById('btn-save').addEventListener('click', () => host.saveProject());
document.getElementById('btn-open').addEventListener('click', () => host.openProject());
btnExport.addEventListener('click', () => host.exportVideo());

const btnSnap = document.getElementById('btn-snap');
btnSnap.addEventListener('click', () => {
  app.snapping = !app.snapping;
  btnSnap.classList.toggle('is-active', app.snapping);
  btnSnap.setAttribute('aria-pressed', String(app.snapping));
  statusText.textContent = app.snapping
    ? 'Snapping on — edges and the playhead pull together'
    : 'Snapping off — edges land exactly where you drop them';
});
